  ...
  return (
    <form className="space-y-6" onSubmit={handleSubmit}>
      ...
      <div className="rta-section">
        ...
        <div className="space-y-4">
            {form.veiculos.map((vehicle, index) => (
                <div key={index} className="p-4 bg-slate-50 border-slate-200 dark:bg-slate-800/50 dark:border-slate-700 rounded-lg border relative">
                    <div className="flex items-center justify-between mb-4">
                        <h5 className="text-sm font-semibold text-slate-700 dark:text-slate-200">Veículo {index + 1}</h5>
                      ...
                      <div className="input-group">
                        ...
                        {(vehicle.marca || vehicle.modelo || vehicle.ano) && (
                          <div className="mt-2 text-xs text-slate-500 dark:text-slate-400 space-y-1">
                            <div><span className="text-slate-700 dark:text-slate-200">Marca:</span> {vehicle.marca}</div>
                            <div><span className="text-slate-700 dark:text-slate-200">Modelo:</span> {vehicle.modelo}</div>
                            <div><span className="text-slate-700 dark:text-slate-200">Ano:</span> {vehicle.ano}</div>
                          </div>
                        )}
                      </div>
                        <div className="input-group">
      ...
      <div className="rta-section">
        ...
        <div className="space-y-4">
            ...
            {form.pessoas.map((driver, index) => (
                <div key={index} className="p-4 bg-slate-50 border-slate-200 dark:bg-slate-800/50 dark:border-slate-700 rounded-lg border relative">
                    <div className="flex items-center justify-between mb-4">
                        <h5 className="text-sm font-semibold text-slate-700 dark:text-slate-200">Driver adicional</h5>
      ...
      <div className="rta-section">
        ...
        <div className="rta-grid rta-grid-auto gap-4">
            <div className="input-group md:col-span-2">
                <input type="file" multiple accept="image/*" onChange={handleFileChange} className="block w-full text-sm text-slate-700 dark:text-slate-200 file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-slate-100 file:text-slate-700 dark:file:bg-slate-800 dark:file:text-slate-200 hover:file:bg-slate-200 dark:hover:file:bg-slate-700" />
            </div>
        </div>
      </div>
